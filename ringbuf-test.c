 ...
 */
void *fill_buffer(void *buf, size_t buf_size, const char *test_pattern) {
...
void sigabort(int unused) {
  cleanup();
  exit(1);
}

#define START_NEW_TEST(test_num) fprintf(stderr, "Test %d...", (++test_num));

#define END_TEST(test_num) fprintf(stderr, "pass.\n");
...
int main(int argc, char **argv) {
  ...
  assert(ringbuf_head(rb1) == rb1_base);
  assert(ringbuf_tail(rb1) == rb1_base + 1);
  assert(strncmp(ringbuf_tail(rb1), (const char *)buf + RINGBUF_SIZE,
                 RINGBUF_SIZE - 1) == 0);
  END_TEST(test_num);
  ...
  ringbuf_memset(rb1, 1, ringbuf_buffer_size(rb1));
  ringbuf_reset(rb1);
  assert(ringbuf_memcpy_into(rb1, buf, strlen(test_pattern)) ==
         ringbuf_head(rb1));
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_free(rb1) ==
         ringbuf_capacity(rb1) - strlen(test_pattern));
  ...
  assert(ringbuf_memcpy_into(rb1, buf, strlen(test_pattern)) ==
         ringbuf_head(rb1));
  assert(ringbuf_memcpy_into(rb1, buf + strlen(test_pattern),
                             strlen(test_pattern) - 1) == ringbuf_head(rb1));
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_free(rb1) ==
         ringbuf_capacity(rb1) - (2 * strlen(test_pattern) - 1));
  assert(!ringbuf_is_full(rb1));
  assert(!ringbuf_is_empty(rb1));
  assert(strncmp((const char *)buf, ringbuf_tail(rb1),
                 2 * strlen(test_pattern) - 1) == 0);
  assert(*(char *)ringbuf_head(rb1) == '\1');
  END_TEST(test_num);
  ...
  assert(ringbuf_memcpy_into(rb1, buf + RINGBUF_SIZE - 2, 1) ==
         ringbuf_head(rb1));
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_free(rb1) == 0);
  assert(ringbuf_is_full(rb1));
  ...
  /* tail should have bumped forward by 1 byte */
  assert(ringbuf_tail(rb1) == rb1_base + 1);
  assert(strncmp(ringbuf_tail(rb1), (const char *)buf + 1, RINGBUF_SIZE - 1) ==
         0);
  END_TEST(test_num);
  ...
  ringbuf_reset(rb1);
  assert(ringbuf_memcpy_into(rb1, buf, RINGBUF_SIZE - 1) == ringbuf_head(rb1));
  assert(ringbuf_memcpy_into(rb1, buf + RINGBUF_SIZE - 1, 1) ==
         ringbuf_head(rb1));
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_free(rb1) == 0);
  assert(ringbuf_bytes_used(rb1) == ringbuf_capacity(rb1));
  ...
  assert(strncmp(ringbuf_tail(rb1), (const char *)buf + 2, RINGBUF_SIZE - 2) ==
         0);
  assert(strncmp((const char *)rb1_base, (const char *)buf + RINGBUF_SIZE, 1) ==
         0);
  END_TEST(test_num);
  ...
  ringbuf_reset(rb1);
  assert(ringbuf_read(rdfd, rb1, strlen(test_pattern)) == strlen(test_pattern));
  assert(ringbuf_read(rdfd, rb1, strlen(test_pattern) - 1) ==
         strlen(test_pattern) - 1);
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_free(rb1) ==
         ringbuf_capacity(rb1) - (2 * strlen(test_pattern) - 1));
  assert(!ringbuf_is_full(rb1));
  ...
  ringbuf_memset(rb1, 1, ringbuf_buffer_size(rb1));
  ringbuf_reset(rb1);
  assert(ringbuf_read(rdfd, rb1, RINGBUF_SIZE + 1) ==
         RINGBUF_SIZE); /* short count */
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_free(rb1) == 0);
  assert(ringbuf_bytes_used(rb1) == ringbuf_capacity(rb1));
  assert(ringbuf_is_full(rb1));
  ...
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_free(rb1) ==
         ringbuf_capacity(rb1) - strlen(test_pattern2));
  ...
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_free(rb1) ==
         ringbuf_capacity(rb1) - (strlen(test_pattern2) - 3));
  assert(ringbuf_bytes_used(rb1) == strlen(test_pattern2) - 3);
  assert(!ringbuf_is_full(rb1));
  assert(!ringbuf_is_empty(rb1));
  assert(ringbuf_tail(rb1) == rb1_base + 3);
  assert(ringbuf_head(rb1) ==
         (uint8_t *)ringbuf_tail(rb1) + (strlen(test_pattern2) - 3));
  assert(strncmp((const char *)dst, test_pattern2, 3) == 0);
  assert(strncmp((const char *)dst + 3, (const char *)buf + 3,
                 RINGBUF_SIZE * 2 - 3) == 0);
  END_TEST(test_num);
  ...
  assert(strncmp((const char *)dst + RINGBUF_SIZE - 1,
                 (const char *)buf + RINGBUF_SIZE - 1, RINGBUF_SIZE + 1) == 0);
  END_TEST(test_num);
  ...
  assert(strncmp((const char *)dst + 13, (const char *)buf + 13,
                 RINGBUF_SIZE * 2 - 13) == 0);
  END_TEST(test_num);
  ...
  assert(ringbuf_memcpy_from(dst + RINGBUF_SIZE - 2, rb1, 1) ==
         ringbuf_tail(rb1));
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_free(rb1) == ringbuf_capacity(rb1));
  assert(ringbuf_bytes_used(rb1) == 0);
  ...
  assert(strncmp((const char *)dst, (const char *)buf2, RINGBUF_SIZE - 1) == 0);
  assert(strncmp((const char *)dst + RINGBUF_SIZE - 1,
                 (const char *)buf + RINGBUF_SIZE - 1,
                 RINGBUF_SIZE * 2 - (RINGBUF_SIZE - 1)) == 0);
  END_TEST(test_num);
  ...
  assert(strncmp((const char *)dst + 14, (const char *)buf + 14,
                 RINGBUF_SIZE * 2 - 14) == 0);
  END_TEST(test_num);
  ...
  assert(strncmp((const char *)dst + 7, (const char *)buf2 + 7,
                 RINGBUF_SIZE * 2 - 7) == 0);
  ...
  assert(ringbuf_read(rdfd, rb1, 11) == 11);
  assert(ringbuf_write(wrfd, rb1, 11) == 11);
  assert(ringbuf_read(rdfd, rb1, RINGBUF_SIZE - 11 - 1) ==
         RINGBUF_SIZE - 11 - 1);
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  ...
  assert(strncmp((const char *)dst + RINGBUF_SIZE - 1,
                 (const char *)buf2 + RINGBUF_SIZE - 1,
                 RINGBUF_SIZE * 2 - (RINGBUF_SIZE - 1)) == 0);
  END_TEST(test_num);
  ...
  assert(strncmp((const char *)dst + RINGBUF_SIZE,
                 (const char *)buf2 + RINGBUF_SIZE, RINGBUF_SIZE) == 0);
  END_TEST(test_num);
  ...
  assert(strncmp((const char *)dst, (const char *)buf, RINGBUF_SIZE + 1) == 0);
  assert(strncmp((const char *)dst + RINGBUF_SIZE + 1,
                 (const char *)buf2 + RINGBUF_SIZE + 1, RINGBUF_SIZE - 1) == 0);
  END_TEST(test_num);
  ...
  assert(ringbuf_memcpy_into(rb1, buf, 11) == ringbuf_head(rb1));
  assert(ringbuf_memcpy_from(dst, rb1, 11) == ringbuf_tail(rb1));
  assert(ringbuf_memcpy_into(rb1, buf + 11, RINGBUF_SIZE - 11 - 1) ==
         ringbuf_head(rb1));
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_free(rb1) == 11);
  assert(ringbuf_bytes_used(rb1) == RINGBUF_SIZE - 11 - 1);
  ...
  assert(ringbuf_memcpy_into(rb1, buf, 11) == ringbuf_head(rb1));
  assert(ringbuf_memcpy_from(dst, rb1, 11) == ringbuf_tail(rb1));
  assert(ringbuf_memcpy_into(rb1, buf + 11, RINGBUF_SIZE - 11) ==
         ringbuf_head(rb1));
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_free(rb1) == 10);
  assert(ringbuf_bytes_used(rb1) == RINGBUF_SIZE - 11);
  ...
  assert(ringbuf_memcpy_into(rb1, buf + 11, RINGBUF_SIZE - 11 + 1) ==
         ringbuf_head(rb1));
  assert(ringbuf_memcpy_from(dst + 11, rb1, RINGBUF_SIZE - 12) ==
         ringbuf_tail(rb1));
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_free(rb1) == RINGBUF_SIZE - 3);
  assert(ringbuf_bytes_used(rb1) == 2);
  ...
  assert(ringbuf_memcpy_into(rb1, buf + 11, RINGBUF_SIZE - 11 + 1) ==
         ringbuf_head(rb1));
  assert(ringbuf_memcpy_from(dst + 11, rb1, RINGBUF_SIZE - 11) ==
         ringbuf_tail(rb1));
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_free(rb1) == RINGBUF_SIZE - 2);
  assert(ringbuf_bytes_used(rb1) == 1);
  ...
  assert(ringbuf_memcpy_into(rb1, buf + 11, RINGBUF_SIZE - 11 + 1) ==
         ringbuf_head(rb1));
  assert(ringbuf_memcpy_from(dst + 11, rb1, RINGBUF_SIZE - 11 + 1) ==
         ringbuf_tail(rb1));
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_free(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_used(rb1) == 0);
  ...
  assert(ringbuf_tail(rb1) == rb1_base + RINGBUF_SIZE - 1);
  /* overflow */
  assert(ringbuf_memcpy_into(rb1, buf + RINGBUF_SIZE, RINGBUF_SIZE - 1) ==
         ringbuf_head(rb1));
  assert(ringbuf_capacity(rb1) == RINGBUF_SIZE - 1);
  assert(ringbuf_bytes_free(rb1) == 0);
  assert(ringbuf_bytes_used(rb1) == RINGBUF_SIZE - 1);
  ...
  assert(strncmp((const char *)ringbuf_tail(rb1) + 3, (const char *)buf2, 2) ==
         0);
  END_TEST(test_num);
  ...
  assert(strncmp(ringbuf_tail(rb1), (const char *)buf + 1, RINGBUF_SIZE - 2) ==
         0);
  assert(strncmp((const char *)ringbuf_tail(rb1) + RINGBUF_SIZE - 2,
                 (const char *)buf2, 1) == 0);
  END_TEST(test_num);
  ...
  assert(ringbuf_head(rb1) == rb1_base + 1);
  assert(ringbuf_head(rb2) == rb2_base + 2);
  assert(strncmp(ringbuf_tail(rb1), (const char *)buf + 2, RINGBUF_SIZE - 3) ==
         0);
  /* last position in contiguous buffer */
  assert(strncmp((const char *)ringbuf_tail(rb1) + RINGBUF_SIZE - 3,
                 (const char *)buf2, 1) == 0);
  ...
  assert(ringbuf_memcpy_into(rb2, (uint8_t *)buf2 + RINGBUF_SIZE - 1, 2) ==
         ringbuf_head(rb2));
  ...
  assert(ringbuf_head(rb2) == rb2_base + 1);
  assert(strncmp(ringbuf_tail(rb1), (const char *)buf2 + RINGBUF_SIZE - 3, 3) ==
         0);
  assert(*(char *)ringbuf_head(rb1) == '\1');
  END_TEST(test_num);
  ...
  assert(ringbuf_memcpy_from(dst, rb2, RINGBUF_SIZE - 3) == ringbuf_tail(rb2));
  assert(ringbuf_tail(rb2) == rb2_base + RINGBUF_SIZE - 3);
  assert(ringbuf_memcpy_into(rb2, buf2 + RINGBUF_SIZE - 1, 2) ==
         ringbuf_head(rb2));
  ...
  assert(ringbuf_head(rb2) == rb2_base + 1);
  assert(strncmp(ringbuf_tail(rb1), (const char *)buf2 + RINGBUF_SIZE - 3, 4) ==
         0);
  assert(*(char *)ringbuf_head(rb1) == '\1');
  END_TEST(test_num);
  ...
  assert(strncmp(ringbuf_tail(rb1), (const char *)buf2 + RINGBUF_SIZE - 3, 3) ==
         0);
  assert(strncmp((const char *)rb1_base, (const char *)buf2 + RINGBUF_SIZE,
                 1) == 0);
  END_TEST(test_num);
  ...
  assert(ringbuf_memcpy_into(rb2, buf2 + RINGBUF_SIZE - 1, 5) ==
         ringbuf_head(rb2));
  assert(ringbuf_memcpy_into(rb1, buf, RINGBUF_SIZE - 3) == ringbuf_head(rb1));
  assert(ringbuf_head(rb1) == rb1_base + RINGBUF_SIZE - 3);
  ...
  /* one byte from buf */
  assert(strncmp(ringbuf_tail(rb1), (const char *)buf + RINGBUF_SIZE - 4, 1) ==
         0);
  /* 5 bytes from buf2, 3 at end of contiguous buffer and 2 after the wrap */
  assert(strncmp((const char *)ringbuf_tail(rb1) + 1,
                 (const char *)buf2 + RINGBUF_SIZE - 2, 3) == 0);
  assert(strncmp((const char *)rb1_base,
                 (const char *)buf2 + RINGBUF_SIZE - 2 + 3, 2) == 0);
  END_TEST(test_num);
  ...
  assert(ringbuf_head(rb1) == rb1_base + 1);
  assert(ringbuf_head(rb2) == rb2_base + 2);
  assert(strncmp(ringbuf_tail(rb1), (const char *)buf + 2,
                 RINGBUF_SIZE - 1 - 2) == 0);
  assert(strncmp((const char *)ringbuf_tail(rb1) + RINGBUF_SIZE - 1 - 2,
                 (const char *)buf2, 1) == 0);
  assert(strncmp((const char *)rb1_base, (const char *)buf2 + 1, 1) == 0);
  END_TEST(test_num);
  ...
  ringbuf_memset(rb1, 1, ringbuf_buffer_size(rb1));
  ringbuf_reset(rb1);
  assert(ringbuf_memcpy_into(rb1, buf, strlen(test_pattern) + 1) ==
         ringbuf_head(rb1));
  ...
  assert(ringbuf_memcpy_into(rb1, buf, strlen(test_pattern) + 1) ==
         ringbuf_head(rb1));
  assert(ringbuf_findchr(rb1, 'a', strlen(test_pattern)) ==
         strlen(test_pattern));
  END_TEST(test_num);
  ...
  assert(ringbuf_memcpy_into(rb1, buf, strlen(test_pattern) + 1) ==
         ringbuf_head(rb1));
  assert(ringbuf_findchr(rb1, 'a', strlen(test_pattern) + 1) ==
         ringbuf_bytes_used(rb1));
  END_TEST(test_num);
  ...
  ringbuf_memset(rb1, 1, ringbuf_buffer_size(rb1));
  ringbuf_reset(rb1);
  assert(ringbuf_memcpy_into(rb1, buf, (strlen(test_pattern) * 2) - 1) ==
         ringbuf_head(rb1));
  assert(ringbuf_findchr(rb1, 'a', strlen(test_pattern) + 1) ==
         ringbuf_bytes_used(rb1));
  END_TEST(test_num);
